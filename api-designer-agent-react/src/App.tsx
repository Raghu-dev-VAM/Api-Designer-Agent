...
import {
  extractRequirementsFromFile, generateOpenApi, generatePostmanCollection,
  generateDataModels, generateSwaggerDocs, fetchAzureStories, fetchJiraStories,
  fetchConfluenceStories,
} from './services/documentService';
import type { AzureConfig, JiraConfig, ConfluenceConfig, ColumnMapping } from './services/documentService';
...
export default function App() {
  ...
  const handleUploadFile = async (file: File, mapping?: ColumnMapping) => {
    setUploading(true);
    setToast(`Extracting requirements from ${file.name}…`);
    try {
      const result = await extractRequirementsFromFile(file, mapping);
      ...
      mergeRequirements(result.requirements);
      setSelectedSourceIds((cur) => cur.includes('docx') ? cur : [...cur, 'docx']);
      setToast(`Extracted ${result.requirements.length} requirements from ${file.name}`);
      setActivity((cur) => [{ label: 'Uploaded', value: file.name }, ...cur.slice(0, 2)]);
    } catch (err: unknown) {
      setToast(err instanceof Error ? err.message : 'Extraction failed');
      ...
      {excelPendingFile && excelColumns.length > 0 && (
        <ExcelColumnMapModal
          ...
          onConfirm={(mapping) => {
            ...
            setExcelColumns([]);
            handleUploadFile(f, mapping);
          }}
          onCancel={() => { setExcelPendingFile(null); setExcelColumns([]); }}
        />
      )}

      {uploading && <Loader message="Extracting requirements from file…" />}
