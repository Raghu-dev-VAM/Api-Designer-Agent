...
const FIELDS: { key: keyof ColumnMapping; label: string; required: boolean; hint: string }[] = [
  ...
  { key: 'title',              label: 'Title / Epic',             required: false, hint: 'Short title or epic name' },
  ...
  { key: 'epic',               label: 'Epic',                     required: false, hint: 'Epic or feature group' },
];
...
const PATTERNS: Record<keyof ColumnMapping, string[]> = {
  ...
  acceptanceCriteria: ['acceptance', 'criteria', 'ac', 'done'],
  epic:               ['epic', 'feature', 'module', 'category'],
};

function autoDetect(columns: string[]): ColumnMapping {
  const mapping: ColumnMapping = { storyId: '', title: '', userStory: '', priority: '', acceptanceCriteria: '', epic: '' };
      ...
      <div className="azure-modal" style={{ maxWidth: 520 }}>
        ...
        <div className="azure-modal-body">
          <p style={{ margin: 0, fontSize: 12.5, color: '#64748b' }}>
            We detected <strong>{columns.length} columns</strong>. Map them to requirement fields below.
            Auto-detected matches are pre-filled — adjust as needed.
          </p>
