...
r = client.post('/api/auth/register', json={'username': 'alice', 'email': 'alice@test.com', 'password': 'pass123'})
check('POST /api/auth/register is public -> 201', r.status_code == 201)

r = client.post('/api/auth/login', data={'username': 'alice', 'password': 'pass123'})
...
r = client.post('/api/auth/register', json={'username': 'bob', 'email': 'alice@test.com', 'password': 'pass123'})
check('Duplicate email -> 400', r.status_code == 400, r.json().get('detail', ''))

r = client.post('/api/auth/register', json={'username': 'bob', 'email': 'bob@test.com', 'password': 'pass123'})
...
r = client.post('/api/auth/login', data={'username': 'nobody', 'password': 'pass123'})
...
check('POST /api/azure/fetch-stories with token -> auth passed (not 401)', r.status_code != 401, str(r.status_code))
